  ...
  useEffect(() => {
    const fetchData = debounce(async () => {
      try {
        ...
        if (searchTerm.trim() === "") {
          ...
          setDigimons(digimonsData.map((res) => ({ ...res.data, showContent: false })));
        } else {
          ...
          const detailedDigimons = await Promise.all(
            searchDigimons.map((searchDigimon) => axios.get(searchDigimon.href))
          );
          const detailedDigimonData = detailedDigimons.map((res) => ({
            ...res.data,
            showContent: false,
          }));
	...
	const toggleCardContent = (index) => {
		setDigimons((prevDigimons) =>
			prevDigimons.map((digimon, i) => 
				({ ...digimon, showContent: i === index ? !digimon.showContent : false })
			)
		);
	};
      ...
      {digimons.length > 0 ? (
        <div className="card-wrapper">
          {digimons.map((digimon, index) => (
            <div
              className={`digimon-card ${digimon.showContent ? 'active' : ''}`}
              key={digimon.id}
            ...
            >
              ...
              <div className={`digimon-card-content ${digimon.showContent ? 'active' : ''}`}>
                ...
                {digimon.attributes && digimon.attributes.length > 0 && (
                  <p>
                    Attribute: {digimon.attributes.map((attribute) => attribute.attribute).join(", ")}
                  </p>
                )}
                <h3>Description:</h3>
